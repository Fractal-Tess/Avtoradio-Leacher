<!-- Make she the bottom rounded value of fixed elements is the same as  html/body in the app.scss file  -->

<div
  class="w-full rounded-b-3xl h-6 items-center flex justify-around text-sm shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.1)]"
>
  <a target="_blank" href="https://github.com/Fractal-Tess">
    Made by <span class="text-secondary-content bg-secondary p-[2px] rounded-md"
      >Fractal-Tess</span
    >
  </a>

  <a
    target="_blank"
    href="https://github.com/Fractal-Tess/Avtoradio-Leacher"
    class="border-b-2 border-secondary">GitHub</a
  >
</div>
